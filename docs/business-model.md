# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Tunisia

## Classification

- Repository: `cloud-itonami-iso3166-tun`
- ISO 3166: `TUN` (Tunisia)
...
- Social impact: [:tunisian-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Tunisian
  public contract
- a foreign SME or civic-tech vendor entering the public sector in
  Tunisia for the first time
- a `cloud-itonami-M6910` client that has just completed incorporation and
  now needs public-sector market access

## Offer

- registration walkthrough for TUNEPS, Tunisia's electronic
  public-procurement platform, including remote enterprise-registration
  and register-extract retrieval integration with the national business
  register
- business/tax registration checklist: an entry in the Registre National
  des Entreprises (RNE), Tunisia's central business database managed by
  the Centre national du registre des entreprises (CNRE, established
  2018), covering merchants, non-commercial professions and beneficial
  owners
- local-content / preferential-procurement navigation: Tunisian
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Tunisian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Tunisia).
