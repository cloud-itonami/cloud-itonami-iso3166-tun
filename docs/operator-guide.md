...
2. Register the client's intake: business type, target public function,
   prior filing history in Tunisia if any.
3. Run the advisor in read-only mode against TUNEPS.
...
- named referral relationship with Tunisian-licensed counsel or a
...
a working referral relationship with Tunisian-licensed counsel or a
registered agent for whatever licensed representation the law of
Tunisia requires for actual public-procurement filings.
