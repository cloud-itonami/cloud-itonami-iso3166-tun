(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Tunisia-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Tunisia-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-tn-entity?
                                       true` (near-universal for Tunisia
                                       ...
                                       Grounded in Tunisian legal entity
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(RNE/統一資格/TUNEPS/HAICOP登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- tn-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-tn-entity? true`, INDEPENDENTLY verify
  `:has-tn-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-tn-entity?` ground truth (most Tunisia public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-tn-entity? e))
                 (not (true? (:has-tn-entity? e))))
        [{:rule :tn-entity-missing
          :detail (str subject " はチュニジア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- rne-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-rne? true`, INDEPENDENTLY check
  `:rne-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-rne? e))
                 (not (true? (:rne-verified? e))))
        [{:rule :rne-unverified
          :detail (str subject " はRNE確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (tn-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (rne-unverified-violations request st)
