(ns marketentry.marketentryllm
  ...
  the TUN public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-tn-entity?=" (:has-tn-entity? e)
                        " rne-verified?=" (:rne-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-tn-entity? e))
                              (:has-tn-entity? e))
                          (or (not (:requires-rne? e))
                              (:rne-verified? e)))
                   0.9 0.3)}))
